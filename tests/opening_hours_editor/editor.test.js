...
import OpeningHoursEditor from '../../components/opening_hours_editor/editor';
import OpeningHoursInterval from '../../components/opening_hours_editor/interval';
...
describe('OpeningHoursEditor', () => {
  ...
  it('allow to choose and display the interval', async () => {
    ...
    expect(editor.findComponent(OpeningHoursInterval).exists()).toBe(true);
    editor.vm.interval = ['08:00', '12:00'];
    editor.vm.closeDialog();
    await Vue.nextTick();
    expect(editor.findComponent(OpeningHoursInterval).exists()).toBe(false);
